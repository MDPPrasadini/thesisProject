...
async function upload() {
  ...
  const segments = state.activeTranscript;
  if (!segments && !fileInput.files.length) {
  ...
  xhr.onload = function () {
    try {
      ...
      ((state.activeLecture = file.name), setStatus("Upload complete ✅"));
...
async function loadSample(filename) {
  try {
    console.log("Clicked sample:", filename);

    setStatus("Loading sample lecture...");
    showLoader("Loading sample lecture...");
    ((state.activeLecture = null),
      (state.activeTranscript = null),
      (state.issample = false),
      (state.mainSummaryCache = null));
    state.searchSummaryCache = {};
    state.aiSummaryCache = {};
    const videoUrl = `${API}/samples/${filename}`;
    ...
    const res = await fetch(`${API}/sample-transcribe`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
